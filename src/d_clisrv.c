...
static boolean SV_SendServerConfig(INT32 node)
{
	...
	CV_SaveNetVars(&p, false);
	{
		...
		if (debugfile)
		{
			fprintf(debugfile, "ServerConfig Packet about to be sent, size of packet:%s to node:%i\n",
				sizeu1(len), node);
		}
#endif
...
void CL_RemovePlayer(INT32 playernum, INT32 reason)
{
	...
	// Reset the name
	sprintf(player_names[playernum], "Player %i", (playernum+1));
...
boolean TryRunTics(tic_t realtics)
{
	...
	if (debugfile && (realtics || neededtic > gametic))
	{
		...
		fprintf(debugfile, "------------ Tryruntic: REAL:%u NEED:%u GAME:%u LOAD: %i\n",
